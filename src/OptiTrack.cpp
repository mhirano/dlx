...
int OptiTrack::run() {
	...
	SPDLOG_INFO("Launching NatNet Sample Client (NatNet ver. {}.{}.{}.{})", ver[0], ver[1], ver[2], ver[3]);
	...
	g_pClient->SetFrameReceivedCallback(OptiTrack::to_DataHandler, this);	// this function will receive data from the server
	...
	if (1)
	{
		...
		SPDLOG_DEBUG("Looking for NatNet servers on the local network.");

		NatNetDiscoveryHandle discovery;
		NatNet_CreateAsyncServerDiscovery(&discovery, &OptiTrack::to_ServerDiscoveredCallback, this);
		SPDLOG_DEBUG("Waiting to connect to NatNet server.");
		std::this_thread::sleep_for(std::chrono::milliseconds(100));
		SPDLOG_DEBUG("Number of discovered servers: {}", g_discoveredServers.size());

		while (true)
		{
			const size_t serverIndex = '1' - '1';
			if (serverIndex < g_discoveredServers.size())
			{
				...
				{
					SPDLOG_DEBUG("Connected to NatNet server");
				...
				{
					SPDLOG_DEBUG("Connected to NatNet legacy server");
			...
			else { // No OptiTrack server found.
				SPDLOG_ERROR("No valid OptiTrack server found.");
				NatNet_FreeAsyncServerDiscovery(discovery);
				return 1;
			}
		}
	...
	while (!bExit)
	{
		auto md = appMsg->optiTrackTerminationRequestMessenger->receive();
		if (md != nullptr) {
			if (md->isRequested == true) {
				printf("\n###############################TERMINATION SIGNAL RECEIVED IN OPTITRACK'S LOOP#####################################\n");
				break;
			}
			else {
				printf("\n###############################TERMINATION SIGNAL RECEIVED IN OPTITRACK'S LOOP BUT FALSE#####################################\n");
			}
...
bool OptiTrack::getRigidBodyIDByName(std::string rigidBodyName) {
	printf("\n\n[SampleClient] Requesting Data Descriptions...\n");
	sDataDescriptions* pDataDefs = NULL;
	int iResult = g_pClient->GetDataDescriptionList(&pDataDefs);
	if (iResult != ErrorCode_OK || pDataDefs == NULL)
	{
		SPDLOG_ERROR("Rigid body name {} is not avaialble.", rigidBodyName);
		return false;
	}
	else
	{
		for (int i = 0; i < pDataDefs->nDataDescriptions; i++)
		{
			...
			{
				sRigidBodyDescription* pRB = pDataDefs->arrDataDescriptions[i].Data.RigidBodyDescription;
				if (pRB->szName == rigidBodyName) {
					printf("Specified rigid body name found\n");
					printf("RigidBody Name : %s\n", pRB->szName);
					printf("RigidBody ID : %d\n", pRB->ID);
				}
			}
...
void NATNET_CALLCONV OptiTrack::ServerDiscoveredCallback(const sNatNetDiscoveredServer* pDiscoveredServer)
{
	char serverHotkey = '.';
...
void NATNET_CALLCONV OptiTrack::DataHandler(sFrameOfMocapData* data)
{
	sDataDescriptions* pDataDefs = NULL;
	int iResult = g_pClient->GetDataDescriptionList(&pDataDefs); // Retrieve data descriptions
	if (iResult != ErrorCode_OK || pDataDefs == NULL)
	{
		SPDLOG_ERROR("[SampleClient] Unable to retrieve Data Descriptions.");
		std::cerr << "[SampleClient] Unable to retrieve Data Descriptions." << std::endl;
	}
	else {
		static bool isVerbose = false;
		auto optiTrackVerboseMsg = appMsg->optiTrackVerboseMessenger->receive();
		if (optiTrackVerboseMsg != nullptr) {
			isVerbose = optiTrackVerboseMsg->isVerbose;
		}
		if (!isVerbose) {
			/*
			 * Sending retrieved data. See AppMsg.h for the destination.
			 */
			OptiTrackDataMsg *dm;
			dm = appMsg->optiTrackDataMessenger->prepareMsg();
			dm->optiTrackData = std::move(*data);
			dm->optiTrackDataDescriptions = std::move(*pDataDefs);
			//dm->optiTrackDataDescriptions.nDataDescriptions = pDataDefs->nDataDescriptions;
			//for (int i = 0; i < pDataDefs->nDataDescriptions; i++) {
			//	dm->optiTrackDataDescriptions.arrDataDescriptions[i].type = pDataDefs->arrDataDescriptions[i].type;
			//	dm->optiTrackDataDescriptions.arrDataDescriptions[i].Data.RigidBodyDescription = std::move(pDataDefs->arrDataDescriptions[i].Data.RigidBodyDescription);
			//}
			appMsg->optiTrackDataMessenger->send();
		} else {
			/*
			 * Print messages as in the sample code
			 */
			...
			if (bSystemLatencyAvailable)
			{
				...
				const double clientLatencyMillisec = g_pClient->SecondsSinceHostTimestamp(data->CameraMidExposureTimestamp) * 1000.0;
...
void NATNET_CALLCONV OptiTrack::MessageHandler(Verbosity msgType, const char* msg)
{
...
void OptiTrack::_WriteHeader(FILE* fp, sDataDescriptions* pBodyDefs)
{
	int i = 0;
...
void OptiTrack::_WriteFrame(FILE* fp, sFrameOfMocapData* data)
{
...
void OptiTrack::_WriteFooter(FILE* fp)
{
	fprintf(fp, "</Data>\n\n");
	fprintf(fp, "</MarkerSet>\n");
}
